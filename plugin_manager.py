...
class PluginManager:
    ...
    async def get_all(self, guild):
        try:
            plugin_names = await self.db.redis.smembers('plugins:{}'.format(guild.id))
            plugins = []
            for plugin in self.mee6.plugins:
                ...
                if plugin.__class__.__name__ in plugin_names:
                    if hasattr(plugin, 'buff_name'):
                        buff_ok = await self.db.redis.get('buffs:{}:{}'.format(guild.id,
                                                                               plugin.buff_name))
                        if not buff_ok:
                            continue
            ...
            return await self.get_all(guild)